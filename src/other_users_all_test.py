...
def test_users_all_two_people(get_new_user_1, get_new_user_detail_1, get_new_user_2, get_new_user_detail_2):
    # Create person one
    person_one, token1 = create_person_one(get_new_user_1, get_new_user_detail_1)
    # Create person two
    ...
    output = other.users_all(token1)['users']
    matches = 0
    for user in output:
        if user in (person_one, person_two):
            matches += 1
    assert matches == 2
