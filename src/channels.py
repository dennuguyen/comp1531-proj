...
def channels_list(*, token):
    ...
    channels_user_in = filter(
        lambda channel: user.get_u_id() in channel.get_u_id_list(), channels)
...
def channels_create(*, token, name, is_public):
    ...
    u_id_list = [u_id] if u_id == 0 else [0, u_id]
    owner_u_id_list = [u_id] if u_id == 0 else [0, u_id]
