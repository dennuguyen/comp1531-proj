...
# unpin successfully
def test_message_unpin(get_new_user_1):
    ...
    # Unpin this message  
    message.message_unpin(token=token1,message_id=msg_id)
    
    # Check if unpinned successfully
    message_object = get_data().get_message_with_message_id(msg_id)
    assert not message_object.get_is_pinned()

    get_data().reset()

# Unpin an invalid message
def test_message_unpin_invalid_message(get_new_user_1):
    ...
    with pytest.raises(error.InputError):
        message.message_unpin(token=token1,message_id=msg_id+1)

    get_data().reset()

# Unpinned by a user who is not the owner
def test_message_unpin_not_by_owner(get_new_user_1, get_new_user_2):
    ...
    # Unpin this message
    with pytest.raises(error.InputError):
        message.message_unpin(token=token2,message_id=msg_id)

    get_data().reset()

# Unpin already pinned message
...
# Unpin by a user who is not a member
def test_message_not_by_a_member(get_new_user_1):
    ...
    with pytest.raises(error.AccessError):
        message.message_unpin(token=token1+'1',message_id=msg_id)
